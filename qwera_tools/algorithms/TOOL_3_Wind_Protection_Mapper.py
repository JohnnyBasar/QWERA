...
class wind_protection_classes(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return """
            <h2>Description</h2>
            <p>
            This tool groups multiple raster layers (wind-protection masks) by the first number in their filename (e.g., wind directions coming from Tool 2) and computes, for each group, a new raster that contains the cell-wise maximum across all group members.
            It is typically applied after <i>Tool 2: Wind Shade Calculator</i> to merge wind shadow masks into composite wind-protection layers. The tool scans for raster files (default: <code>*.tif</code>), optionally including subfolders. each file’s name is inspected for the <i>first numeric substring</i>; all grids sharing that number form one group (e.g., <code>r45a_...</code> → group 45).
            </p>

            <h2>Standards & References</h2>
            <dt><ul>
            <li><b><a href="https://www.sciencedirect.com/science/article/pii/S2215016124004576">Funk &amp; V&ouml;lker (2024)</a></b>, “A GIS-toolbox for a landscape structure based Wind Erosion Risk Assessment (WERA)” — describes the methodological context of using DWD wind statistics for erosion risk modelling. Within every group, the tool calculates a new raster whose pixel values are the maximum across all inputs of that group. all group results are again merged into one overall maximum raster (<code>wind_protection.tif</code>).
